...
def logistic_expected_goals(shots, solver, cw):
    ...
    model = LogisticRegression(max_iter=6000, class_weight=cw,solver=solver)
    model.fit(X_train, y_train)

    score = model.score(X=X_test,y=y_test)
    roc = roc_auc_score(y_test,model.predict_proba(X_test)[:,1])*100
    if(score > 0.875 and roc > 75):
        print('THIS ITERATION IS WITH SOLVER', solver)
        print('THIS ITERATION IS WITH CLASS_WEIGHT', cw)
        print('The accuracy of classifying whether a shot is goal or not is {}%'.format(round(score*100), 2))
...
if __name__ == "__main__":
    ...
    #find_best_hyperparameters(shots)